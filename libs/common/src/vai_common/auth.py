...
지금은 대칭키(HS256) 자체 발급이다. 고객사 SSO(OIDC/SAML) 연동을 붙일 때
이 모듈의 :func:`verify_access_token`만 교체하면 되도록, 나머지 코드는
:class:`Principal`만 알고 토큰 형식을 모른다.

**공통 라이브러리에 두는 이유.** 게이트웨이와 운영 콘솔이 각자 토큰을 검증한다.
블록은 서로를 import 하지 않으므로, 여기 없으면 JWT 구현이 두 벌이 되고
스코프 해석이 갈라진다 — 인증에서 구현이 갈라지는 것은 곧 권한 우회다.
