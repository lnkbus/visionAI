...
```
http://localhost:8094        (뒤 경로는 안 외워도 된다 — 넘어간다)
```

1. **회의 시작**을 누른다. 마이크를 허용하면 그 자리에서 회의가 열린다.
2. 자막이 실시간으로 흐르고, 화자 라벨은 **뒤따라 붙는다** — 화자분리가
   인식보다 느려서 자막을 기다리게 하지 않기로 한 설계다.
3. `speaker_1` 로 나온 화자에 **실명을 붙인다.** 화자분리는 "다른 사람"까지만
   알려 주고 "누구인지"는 모른다. 입력한 이름은 재기동을 넘어 남는다.
4. **회의 종료**를 누른다. 그래야 요약이 돈다 — 창을 닫는 것으로는 안 된다.
   잠시 뒤 [회의록 불러오기].

### 4.1 여러 사람이 각자 기기로 붙기

회의를 시작하면 화면에 **참여 주소**가 나온다. 다른 참석자가 그 주소로 들어와
[회의 참여]를 누르면 같은 회의에 붙는다. 마이크가 여럿이어도 자막은 한
타임라인에 모이고, 회의록도 한 벌만 나온다.

참여 길이 없으면 각자 누를 때마다 회의가 새로 생기고 **회의록이 인원수만큼
쪼개진다.** 상담 데모(8080)도 같은 방식이고, 거기서는 참여하는 쪽이
"상담원 채널"을 골라 양쪽 발화를 한 화면에서 본다.

오디오가 PBX·회의 시스템에서 들어오는 구성이라면 화면이 마이크를 잡을 이유가
없다. `VAI_MEETUI_MIC_ENABLED=false` 로 참관 전용으로 만든다 — 시작 버튼이
사라진다(눌리는데 아무 일도 안 나는 버튼보다 없는 편이 낫다).
