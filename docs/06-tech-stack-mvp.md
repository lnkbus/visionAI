...
│   ├── common/    ✅              # 설정·로깅·버스·H/W지문·라이선스 검증·워커 골격·배포 설정 채널
...
- **라이선스 폐기(revocation) 목록이 없다.** 폐쇄망은 폐기 목록을 받을 경로가
  ...
  갱신하는 방식이 유일한 수단이다.
- **용량 한도는 라이선스에 실리지만 아직 강제되지 않는다.** `concurrent_channels`
  값은 `.lic`에 담기고 `LicenseGate.require(...).limit(...)`으로 읽히지만, 실제 동시
  채널 수를 세어 차단하는 지점은 CORE-BUS 세션 수용부에 붙여야 한다(미구현).
...
2. `CORE-SEC`: KCMVP 검증모듈 연동(ARIA/AES-256), append-only 감사 로그
3. `CORE-ADM`: 운영 콘솔 (테넌트/권한, 블록 상태, GPU 자원, 세션 이력)
4. 에어갭 번들: `build_bundle.sh`(블록별 최소 반입), `install.sh`, 자가진단
5. Helm 차트 + K3s 단일노드 구성 (03 문서 §2)
6. 온프렘 릴리스 빌드: Cython 컴파일 + PyArmor 난독화 + **공개키 내장**
   (`libs/common/src/vai_common/keys/license_pub.pem`) — 이 단계를 거쳐야 서명 검증이 켜진다
7. `RAG-KB` 문서 파서: PDF/DOCX/**HWP**(공공 조달 필수)
