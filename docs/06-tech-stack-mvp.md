...
| AUD-RTP(전화 인입)·UI-MEET·SPK-DIA(화자분리) | 예정 |
...
- **STT 사전의 디코딩 편향은 어댑터 연결이 남았다.** 후처리 교정(등록된 오인식
  표기 치환)은 동작하지만, `initial_prompt`를 Faster-Whisper 호출에 넘기는
  배선은 아직이다. 등록되지 않은 새 변형은 여전히 놓친다.
- **사전 반영에 최대 30초 지연이 있다.** 실시간 경로가 발화마다 저장소를 때리지
  않도록 캐시 TTL을 뒀기 때문이다. 배포 알림(Pub/Sub)으로 즉시 무효화하는
  경로는 구현되어 있으나 STT 워커가 아직 구독하지 않는다.
