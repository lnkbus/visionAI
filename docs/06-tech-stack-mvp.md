...
| AUD-RTP(전화 인입)·UI-AGENT·UI-MEET | 예정 |
| 시나리오 흐름 저작(분기·기간계 액션)·시뮬레이터 — 룰 저작만 구현됨 | 예정 |
| STT 사전의 인식 반영 — 등록·관리만 되고 STT 어댑터 연동은 미구현 | 예정 |
...
- **STT 커스텀 사전은 등록·관리까지만 구현됐다.** 등록한 표기가 실제 인식에
  반영되려면 STT 어댑터가 사전을 읽어 초기 프롬프트/후처리 교정에 쓰는 연결이
  필요하다(다음 스프린트). 지금 등록해도 인식 결과는 바뀌지 않는다.
...
Wave 4에서 `LLM-SUM`과 `SCN-STUDIO`가 완료되었다. 남은 AICC 패키지 조각:

1. `UI-AGENT`: 상담원 워크스페이스 (자막·팝업·컴플라이언스 경고·요약 확인·**팝업 채택 피드백 버튼**)
2. `AUD-RTP`: SIP/RTP 인입, Stereo 채널 분리(고객/상담원)
3. `TA-ASSIST` 추천 답변 생성: 검색 근거 + sLLM 답변 초안 (인용 강제, 근거 없으면 생성 안 함)
4. **STT 사전 → 인식 반영**: STT 어댑터가 사전을 읽어 initial_prompt / 후처리 교정에 적용
5. `RAG-KB` 문서 파서 어댑터: PDF/DOCX/**HWP**(공공 필수)
6. 룰셋 API 배포: SCN-STUDIO 배포 → FLT-MICRO 자동 반영 (현재는 산출물 생성까지)
7. 평가셋 구축: 카드/보험 약관 Q&A 골든셋 100문항 → CI 야간 회귀
8. `LRN-STUDIO` 분리: 오인식·오검색 사례 큐, 재학습 데이터 적재
